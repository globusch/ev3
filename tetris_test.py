#!/usr/bin/env python3
"""
Tetris für LEGO EV3 mit ev3dev2
...
"""Prüft ob eine Position gültig ist."""
for (dr, dc) in shape:
r, c = row + dr, col + dc
...
"""Löscht volle Zeilen und gibt Anzahl zurück."""
...
# Score & Level
info_x = side_x + 4
draw.text((info_x, 4), f"Score", fill='black')
draw.text((info_x, 14), f"{score}", fill='black')
draw.text((info_x, 30), f"Lvl", fill='black')
draw.text((info_x, 40), f"{level}", fill='black')
